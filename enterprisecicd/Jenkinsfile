...
// Test case: Enterprise CI/CD with Maven, SonarQube, deployment approval, and comprehensive post-actions
pipeline {
  // run on jenkins nodes that has java 8 label
  agent { label 'java8' }
          ...
          if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
            ...
            timeout(time: 2, unit: 'MINUTES') {
              ...
              if (developmentArtifactVersion != null && !developmentArtifactVersion.isEmpty()) {
                ...
                // NOTE : CREATE your deployment JOB, where it can take parameters which is the jar name to fetch from jenkins workspace
      ...
      steps {
        script {
          if (currentBuild.result == null || currentBuild.result == 'SUCCESS') {
            ...
            timeout(time: 3, unit: 'MINUTES') {
              // deployment job which will take the released version
              if (releasedVersion != null && !releasedVersion.isEmpty()) {
                // make the application name for the jar configurable
                ...
                // the deployment should be based into Nexus repo
